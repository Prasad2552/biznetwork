    ...
    try {
        ...
        if (thumbnailFile) {
         ...
         const [fileUrl, thumbnailUrl] = await Promise.all([
          uploadFileToS3(file, userId, 'demo'),
            thumbnailFile ? uploadFileToS3(thumbnailFile, userId, 'demo_thumb') : Promise.resolve(null),
        ]);
         ...
         const newDemo = new Video({
          ...
          await newDemo.save();
            ...
            return NextResponse.json({ videoId: newDemo._id, title, channelId }, { status: 201 });
        } catch (error) {
