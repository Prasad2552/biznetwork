    ...
    try {
        ...
        if (thumbnailFile) {
         ...
         const [fileUrl, thumbnailUrl] = await Promise.all([
          uploadFileToS3(file, userId, 'podcast'),
            thumbnailFile ? uploadFileToS3(thumbnailFile, userId, 'podcast_thumb') : Promise.resolve(null),
        ]);
        ...
        const newPodcast = new Video({
            ...
            status: 'draft',
              duration,
              categories: parsedCategories, // Now parsing the categories
               type: contentType === 'podcasts' ? 'podcast' : 'video' , // Setting type to 'podcast'
          ...
          await Channel.findByIdAndUpdate(channelId, { $inc: { videoCount: 1 } });
            ...
            return NextResponse.json({ videoId: newPodcast._id, title, channelId }, { status: 201 });
        } catch (error) {
