    ...
    try {
        ...
        if (thumbnailFile) {
         ...
         const [fileUrl, thumbnailUrl] = await Promise.all([
           uploadFileToS3(file, userId, 'testimonial'),
            thumbnailFile ? uploadFileToS3(thumbnailFile, userId, 'testimonial_thumb') : Promise.resolve(null),
        ]);
          ...
          const newTestimonial = new Video({
          channelId: new Types.ObjectId(channelId),
            ...
            status: 'draft',
              ...
              type: 'testimonial', // Explicitly set the type to testimonial
             slug: slugify(title) // Added slug here
          });
          await newTestimonial.save();

                await Channel.findByIdAndUpdate(channelId, { $inc: { videoCount: 1 } });
            return NextResponse.json({ videoId: newTestimonial._id, title, channelId }, { status: 201 });
        } catch (error) {
