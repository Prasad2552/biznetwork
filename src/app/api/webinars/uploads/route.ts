...
import Video from '@/lib/models/Video'; //Using Video Model
       ...
       const formData = await req.formData();
        ...
        if (thumbnailFile) {
         ...
         const [fileUrl, thumbnailUrl] = await Promise.all([
            uploadFileToS3(file, userId, 'webinars'),
            thumbnailFile ? uploadFileToS3(thumbnailFile, userId, 'webinar_thumb') : Promise.resolve(null),
         ]);
        const newWebinar = new Video({
            ...
            status: 'draft',
              duration,
              type: contentType === 'webinars' ? 'webinar' : 'video',
             slug: slugify(title) // Added slug here
          });
          await newWebinar.save();

                await Channel.findByIdAndUpdate(channelId, { $inc: { videoCount: 1 } });
            return NextResponse.json({ videoId: newWebinar._id, title, channelId }, { status: 201 });
        } catch (error) {
            console.error('Error saving files or updating count:', error);
            return NextResponse.json({ error: 'Failed to save files or update channel count' }, { status: 500 });
        }
    } 