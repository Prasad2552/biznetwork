...
export async function POST(req: NextRequest) {
    try {
        ...
        const featuredImageFile = formData.get('featuredImage') as File | null; // Get the featured image as a File
        ...
        // Upload files to S3
         const userId = uuidv4()
           // Upload files to S3
         const featuredImageUrl = await uploadFileToS3(featuredImageFile as File, userId, 'blogpost_featured'); // Upload to S3; Type assertion
        ...
        const newBlogPost = new BlogPost({
            ...
            content,
            channelId,
            featuredImage: featuredImageUrl,  //Store S3 URL
            createdAt: new Date(),
            updatedAt: new Date(),
            slug,
        ...
        const savedBlogPost = await newBlogPost.save(); // Ensure you await the save

        if (!savedBlogPost) {
            throw new Error("Failed to save blog post");
        }

        await Channel.findByIdAndUpdate(channelId, { $inc: { blogCount: 1 } });

        return NextResponse.json({ postId: savedBlogPost._id }, { status: 201 });
    } catch (error) {
        console.error('Error uploading blog post:', error);
        return NextResponse.json({ message: 'Failed to upload blog post', error: error instanceof Error ? error.message : "Unknown Error" }, { status: 500 });
    }
}